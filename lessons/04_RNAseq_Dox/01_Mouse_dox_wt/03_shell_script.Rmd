...
#SBATCH --mail-type=END,FAIL
  ## note - exit status 0 means it worked, exit status 1 means it failed
...
## These just tell you where it ran, which computer ran it (i.e. Fiji-1 or Fiji-2), and the date finished)
pwd; hostname; date
...
## the backslash "\" means "next line / next command"
## you CANNOT have a space at the end of any line in a bash script, so using "\" at the end of a line is useful
...
## you want to make sure your input data is separated from your output - DON'T direct it to the same place
## for this example class on 2025-02-05, the output directory was: _____
## where your run.sh, nextflow.config file are and do mkdir to make a folder then specify the path
--outdir /scratch/Shares/rinnclass/MASTER_CLASS/STUENTS/<DIRECTORY NAME> \
...
--input design.csv \
## this is basically the same as sample sheet

## We are telling nextflow where our reads/fastq files are (aslo see 01_NF_CORE_RNAseq...Rmd for more)
...
--outdir /scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/eroc4824/MASTER_CLASS/lessons/04_RNAseq_Dox/01_Mouse_dox_wt/dox_long/dox_long_out \
--reads /scratch/Shares/rinnclass/MASTER_CLASS/DATA/mouse_wt_long_timecourse/*{_R1,_R2}.fastq.gz \
--fasta /scratch/Shares/rinnclass/MASTER_CLASS/GENOMES/M25/GRCm38.p6.genome.fa \
--gtf /scratch/Shares/rinnclass/MASTER_CLASS/GENOMES/M25/gencode.vM25.annotation.gtf \
--pseudo_aligner salmon \
...
### note - the top section of this script is the bash portion (the language is bash), but then once you "activate" nextflow, you have to write it in nextflow language format
